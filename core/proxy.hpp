...
public:
    ...
    int getpid() { return coder.get_and_inc_qid(); }
    ...
    int run_single_query(istream &is, istream &fmt_stream, int nopts,
        ...
        if (!fmt_stream.good() && Global::enable_planner) {
            start = timer::get_usec();
        ...
        CostBenchmk b, b_reply;
        b.type = cost_model_t::REAL_COST;
        for (int i = Global::num_servers-1; i >= 0; i--) {
            b.pqid = getpid();
        ...
        uint64_t end_cycle = read_tsc() - start_cycle;
        monitor.finish();
        double read_cost = 0;
        double write_cost = 0;
        ...
        for (int i = 0; i < Global::num_servers; i++) {
            ...
            bundle = adaptor->recv();
            b_reply = bundle.get_cost_benchmk();
            read_cost += b_reply.read_cycle;
            write_cost += b_reply.write_cycle;
        }
        if(Global::use_rdma){
            RDMA &rdma = RDMA::get_rdma();
            read_cost = read_cost * monitor.get_latency(cnt) /
                        end_cycle;
        }
        write_cost = write_cost * monitor.get_latency(cnt) / end_cycle;
        
        if(Global::use_rdma && request.start_from_index()){
    ...
    void run_cost_benchmk(int num, int line_num, int circle_num) {
        vector<ssid_t> stypes, ps, otypes;
        if (stats->get_popular_pattern(stypes, ps, otypes, line_num) == 0) {
            logstream(LOG_ERROR)
                << "Can't generate test query automatically." << LOG_endl;
            return;
        }

        planner.cost_model.init(stypes.size());
        vector<ssid_t> subjects, objects;
        SPARQLQuery big_query;
        int big_query_no = 0;
        for (int i = 0; i < stypes.size(); i++) {
            SPARQLQuery q, q_reply;
            ssid_t stype = stypes[i];
            ssid_t p = ps[i];
            ssid_t otype = otypes[i];
            q.pattern_group.patterns.push_back(
                SPARQLQuery::Pattern(p, PREDICATE_ID, IN, -1));
            q.pattern_group.patterns.push_back(
                SPARQLQuery::Pattern(-1, TYPE_ID, OUT, stype));
            q.pattern_group.patterns.push_back(
                SPARQLQuery::Pattern(-1, p, OUT, -2));
            q.pattern_group.patterns.push_back(
                SPARQLQuery::Pattern(-2, TYPE_ID, OUT, otype));
            q.result.nvars = 2;
            q.result.required_vars.push_back(-1);
            q.result.required_vars.push_back(-2);
            q.result.blind = false;  // must take back the results
            q.pqid = getpid();

            send_request(q);
            q_reply = recv_reply();
            if (q_reply.result.row_num == 0) {
                logstream(LOG_ERROR) << "Try again." << LOG_endl;
                return;
            }
            int choose_row = q_reply.result.row_num / 2;
            subjects.push_back(q_reply.result.get_row_col(choose_row, 0));
            objects.push_back(q_reply.result.get_row_col(choose_row, 1));

            if(i == 0){
                big_query = q_reply;
            } else if(q_reply.result.row_num > big_query.result.row_num){
                big_query = q_reply;
                big_query_no = i;
            }
        }
        ...
        int q_sid = (this->sid + 1) % Global::num_servers;
        int bits_2 = 22;
        // num = 100;
        string data_send = "a";
        ...
        for(int i = 0; i <= bits_2; ++i){
            ...
            for(int j = 0;j < num; j++){
                CostBenchmk request(data_send);
                request.type = cost_model_t::NETWORK;
                request.pqid = getpid();
                Bundle bundle(request);
                send(bundle, q_sid);
                bundle = adaptor->recv();
            }
            uint64_t latency = (timer::get_usec() - start);
            uint64_t all_cycle = read_tsc() - start_cycle;
            double network_latency = adaptor->get_write_cycle() *
                                     latency * 1.0 / all_cycle / num;
            planner.cost_model.add_network_ltcy(network_latency);
        ...
        for (int i = 0; i < stypes.size(); i++) {
            // logstream(LOG_INFO)
            //     << "( " << stypes[i] << " " << subjects[i] << " " << ps[i]
            //     << " " << objects[i] << " " << otypes[i] << " " <<LOG_endl;
            CostBenchmk request(stypes[i], subjects[i], ps[i], objects[i],
                                otypes[i]);
            request.run_times = num;
            request.pqid = getpid();
            Bundle bundle(request);
            int sid = wukong::math::hash_mod(subjects[i], Global::num_servers);
            send(bundle, sid);

            bundle = adaptor->recv();
            ASSERT(bundle.type == COST_BENCHMK);
            reply = bundle.get_cost_benchmk();
            planner.cost_model.add_l2u_sample(i, reply.row_l2u,
                                              reply.latency_l2u);
            planner.cost_model.add_k2u_sample(i, reply.row_k2u_init,
                                              reply.row_k2u_prune,
                                              reply.row_k2u, reply.latency_k2u);
            // init, prune, explore, match
            planner.cost_model.add_k2l_sample(
        ...
        if (stats->get_circle_pattern(tps, clockwise_count, circle_num) == 0) {
            logstream(LOG_ERROR)
                << "Can't generate circle query automatically." << LOG_endl;
            return;
        }
        planner.cost_model.init(tps.size() / 6, model_t::K2K);
        int real_count = 0;
        for (int i = 0; i < tps.size() / 6; i++) {
            CostBenchmk request(tps[6 * i], tps[6 * i + 1], tps[6 * i + 2],
                                tps[6 * i + 3], tps[6 * i + 4], tps[6 * i + 5]);
            // logstream(LOG_INFO)
            //     << tps[6 * i] << " " << tps[6 * i+3] << " " << tps[6 * i+1]
            //     << " "
            //     << tps[6 * i+4] << " " << tps[6 * i+2] << " " << tps[6 * i+5]
            //     << LOG_endl;
            request.run_times = num;
            request.type = (i < clockwise_count) ? (cost_model_t::CLOCKWISE)
                                                   : (cost_model_t::OTHER);
            request.pqid = getpid();
            Bundle bundle(request);
            send(bundle, 0);
            ...
            ASSERT(bundle.type == COST_BENCHMK);
            reply = bundle.get_cost_benchmk();
            if (reply.row_k2k_init != 0) {
                planner.cost_model.add_k2k_sample(
                    real_count, reply.row_k2k_init, reply.row_k2k_prune,
                    reply.row_k2k_bind, reply.row_k2k_result,
                    reply.latency_k2k);
                real_count++;
            }
        }
