...
options_description       plan_desc("plan <args>         plan a SPARQL query");
...
void init_options_desc()
{
    ...
    plan_desc.add_options()
    (",b", value<int>()->value_name("<num>"), "send benchmark request to engine and rewrite cost model factors.")
    (",c", value<int>()->default_value(10)->value_name("<num>"), "circle pattern num to generate k2k factor")
    (",l", value<int>()->default_value(20)->value_name("<num>"), "line pattern num to generate l2u/k2u factor")
    ("help,h", "help message about plan")
    ;
    all_desc.add(plan_desc);
}
...
static void run_sparql(Proxy * proxy, int argc, char **argv)
{
    ...
    if (sparql_vm.count("-f")) {
        ...
        if (!sparql_vm.count("-p") && Global::enable_planner) {
        ...
        } else if (!sparql_vm.count("-p")) {
            logstream(LOG_ERROR)
                << "Optimizer is disabled, "
                << "you must provide a user-defined plan file (-p)."
                << LOG_endl;
            fail_to_parse(proxy, argc, argv);  // invalid cmd
            return;
        } else {
            ...
            if (!fmt_stream.good()) {  // fail to load user-defined plan file
                vector<string> dirs;
                split(dirs, fname, is_any_of("/"));
                // use relative path to find formt file
                string file_name = dirs.back();
                dirs.pop_back();
                string fmt_fname2 = join(dirs, "/") + "/" + fmt_fname + "/" +
                                    file_name + ".fmt";
                fmt_stream.open(fmt_fname2);

                if (!fmt_stream.good()) {
                    logstream(LOG_ERROR)
                        << "The plan file is not found: " << fmt_fname2
                        << LOG_endl;
                    fail_to_parse(proxy, argc, argv);  // invalid cmd
                    return;
                }
...
// static void run_plan(Proxy *proxy, int argc, char **argv) {
//     // use the master proxy thread to store statistics
//     if (!MASTER(proxy)) return;

//     variables_map plan_vm;
//     try {
//         store(parse_command_line(argc, argv, plan_desc), plan_vm);
//     } catch (...) {  // something go wrong
//         fail_to_parse(proxy, argc, argv);
//         return;
//     }
//     notify(plan_vm);
//     // parse options
//     if (plan_vm.count("help")) {
//         if (MASTER(proxy)) cout << plan_desc;
//         return;
//     } else if (plan_vm.count("-b")) {
//         // run benchmark & rewrite cost model factors
//         int num = plan_vm["-b"].as<int>();
//         int circle_num = plan_vm["-c"].as<int>();
//         int line_num = plan_vm["-l"].as<int>();
//         proxy->run_cost_benchmk(num, line_num, circle_num);
//         return;
//     }
// }
...
void run_console(Proxy *proxy)
{
    ...
    if (MASTER(proxy)){
        ...
        proxy->run_cost_benchmk(10, 500, 20);

        uint64_t t2 = timer::get_usec();
        logstream(LOG_INFO) << (t2 - t1) / 1000 << " ms for generating factors" << LOG_endl;
        cout << endl
