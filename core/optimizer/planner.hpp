...
class Planner {
    ...
    CostResult cal_first_pt(ssid_t v1, ssid_t v2, ssid_t d, ssid_t v3, 
        ...
        cost_result.current_model = model_t::I2U;
    ...
    CostResult cal_rest_pt(ssid_t o1, ssid_t p, ssid_t d, ssid_t o2,
                           int mt_factor, int pick, CostResult &old_result,
                           bool from_o2 = false) {
        ...
        } else if (cost_result.path.size() == triples.size()) {
            cost_model.end_network_cost(cost_result, mt_factor);
        }
        // cost_result.print();
        return cost_result;
    }
            ...
            if (o1 < 0 && o2 < 0 && p!=TYPE_ID) {
                ...
                if(!is_end_point(o1, 0)){
                    ...
                    if (contain_k2l.find(o1) == contain_k2l.end()) {
                        CostResult p_result = cal_rest_pt(o1, p, d, o2, r.mt_factor, pt_pick, pto_o1);
                ...
                if (!is_end_point(o2, 0)) {
                    ...
                    if (contain_k2l.find(o2) == contain_k2l.end()) {
                        if(result_bind == 0){
                            CostResult p_result = cal_rest_pt(o2, p, 1-d, o1, r.mt_factor, pt_pick, pto_o2, true);
                            result_bind = p_result.result_bind;
                        }
        ...
        while (!median_results.empty()){
            ...
            for (int pt_pick = 0; pt_pick < _chains_size_div_4; pt_pick++) {
                ...
                if(old_result.is_known(o1)){
                    CostResult new_result = cal_rest_pt(o1, p, d, o2, r.mt_factor, pt_pick, old_result);
                    if (no_result) return;
                    if(min_cost > new_result.all_cost)
                        level_results.push_back(new_result);
                }
                if(old_result.is_known(o2)){
                    CostResult new_result = cal_rest_pt(o2, p, IN, o1, r.mt_factor, pt_pick, old_result, true);
                    if (no_result) return;
                    if(min_cost > new_result.all_cost)
                        level_results.push_back(new_result);
                }
    ...
    bool do_group(SPARQLQuery &r, SPARQLQuery::PatternGroup &group, bool test) {
        ...
        if(!test){
            logstream(LOG_INFO) << "estimate min cost " << min_cost << " usec." << LOG_endl;
        }
        return success;
    }
    ...
    void follow_enum(SPARQLQuery &r, bool print) {
        CostResult cost_result;
        for (int pt_pick = 0; pt_pick < _chains_size_div_4; pt_pick++) {
            int i = 4 * pt_pick;
            ...
            if (pt_pick == 0) {
                if (p != 0 && d == IN) {
                    cost_result =
                        cal_first_pt(o1, p, d, o2, r.mt_factor, pt_pick, true);
                } else {
                    cost_result =
                        cal_first_pt(o1, p, d, o2, r.mt_factor, pt_pick);
                }
            } else {
                // var o1 in the result table
                if (d == IN) {
                    cost_result =
                        cal_rest_pt(o1, p, d, o2, r.mt_factor, pt_pick, cost_result, true);
                } else {
                    cost_result =
                        cal_rest_pt(o1, p, d, o2, r.mt_factor, pt_pick, cost_result);
                }
            }
            if (print) {
                logstream(LOG_INFO)
                    << "network cost " << cost_result.network_cost << LOG_endl;
                cost_result.typetable.print();
            }
        }
        double simple_cost = cost_result.all_cost;
        cost_result.all_cost += cost_result.network_cost;
        logstream(LOG_INFO)
            << "simple cost " << simple_cost << ", network cost "
            << cost_result.network_cost << ", all cost " << cost_result.all_cost << LOG_endl;
    }
   ...
   public:
    ...
    void run_single_path(SPARQLQuery &r, bool print = false) {

        vector<ssid_t> attr_pattern;
        vector<int> attr_pred_chains;
        triples.clear();
        transfer_to_chains(r.pattern_group.patterns, attr_pattern, attr_pred_chains);
