...
public:
    ...
    /**
     * use to calculate network communication factor.
     * return factor
     **/
    double execute_certain_size(SPARQLQuery &r, int size, int test_num=10){
        r.result.keep_part_result(size);
        SPARQLQuery::Result result = r.result;
        RDMA &rdma = RDMA::get_rdma();
        rdma.dev->RestartCycle();
        uint64_t start_cycle = timer::read_tsc();
        ...
        int after_size = r.result.get_row_num();
        uint64_t all_cycle = timer::read_tsc() - start_cycle;
        r.result = result;
        return double(latency) * rdma.dev->read_cycle / all_cycle/ test_num / (size + after_size);
    }

    double execute_test_query(SPARQLQuery &r, int test_num = 1, bool distinct = false, 
                                bool keep_part = true, bool same_server = false) {
        uint64_t latency = 0;
        SPARQLQuery::Result result = r.result;
        for (int i = 0; i < test_num; i++) {
            uint64_t start = timer::get_usec();
            execute_one_pattern(r);
            latency += (timer::get_usec() - start);
            r.pattern_step--;
            r.result = result;
        }
        execute_one_pattern(r);
        ...
        if (keep_part) {
            int subset_size = rand() % SUBSET_SIZE;
            int nrows = subset_size > r.result.get_row_num()
                            ? r.result.get_row_num()
                            : subset_size;
            int tol_row = 0;
            ssid_t start;

            if (r.pattern_step < r.pattern_group.patterns.size()) {
                start = r.get_pattern().subject;              
            } else {
                auto ptn = r.pattern_group.patterns[r.pattern_group.patterns.size() - 1];
                start = ptn.object < 0? ptn.object : ptn.subject;
            }
            vector<sid_t> result_table;
            vector<attr_t> attr_res_table;
            for (int i = 0; i < nrows; i++) {
                int dst_sid = wukong::math::hash_mod(
                    r.result.get_row_col(i, r.result.var2col(start)),
                    Global::num_servers);
                if (dst_sid == sid) {
                    r.result.append_row_to(i, result_table);
                    r.result.append_attr_row_to(i, attr_res_table);
                } else if (!same_server || tol_row < (subset_size / 10)) {
                    tol_row++;
                    r.result.append_row_to(i, result_table);
                    r.result.append_attr_row_to(i, attr_res_table);
                }
            }
            r.result.result_table = result_table;
            r.result.attr_res_table = attr_res_table;

            r.result.update_nrows();
        }
        return double(latency) / test_num;
    }
};
